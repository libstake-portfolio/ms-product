...
- **명세 영역 예외**: `docs/specifications/`는 "일반화"와 "문서는 방향성 만을 서술한다"를 따르지 않는다. 명세는 제품 개념의 실제 이름을 확정적으로 쓰고, 동작과 그 파급을 서술하며, 상태도와 화면 예시를 포함할 수 있다. 코드 식별자, 스키마 이름, 파일 경로는 명세에서도 쓰지 않는다.
...
## 1. 작업 절차 (Working procedure)

> 기능 작업이 어떤 순서로 진행되는지. 각 단계에서 무엇을 기준으로 판단하는지는 위 작업 원칙이 소유한다.

### 1.1 요구사항을 받는다
기능 요구사항은 러프한 상태로 온다. 받은 상태 그대로 구현에 들어가지 않는다.

### 1.2 구체화하고 빈틈을 묻는다
요구사항을 기능 단위로 풀어쓰고, 아직 정해지지 않은 것을 찾아낸다. 모호한 지점은 추측으로 메우지 않고 묻는다(§0.1). 빈틈이 남아 있는 동안에는 다음 단계로 넘어가지 않는다.

### 1.3 명세를 쓴다
채워진 요구사항을 `docs/specifications/`에 명세로 남긴다. 명세는 사용자의 승인으로 확정된다. 명세 없이 만들어진 기능은 근거가 없다. 기능을 바꿀 때도 명세를 먼저 고친다.

### 1.4 명세대로 구현한다
명세에 없는 것을 만들지 않는다. 구현하다가 명세가 답하지 않는 지점을 만나면 코드에서 결정하지 말고 1.2로 돌아간다.

### 1.5 검증한다
성공 기준(§0.5)이 확인될 때까지 반복한다. 타입 검사·정적 검사·서식 검사·테스트·빌드가 모두 통과해야 끝난 것이다.

### 1.6 문서에 반영한다
이번 변경이 명세·아키텍처·결정 기록 중 어디를 바꾸는지 판정하고, 연쇄 반영까지 마친다(§0.9).

### 1.7 보고한다
남긴 것과 남기지 못한 것을 함께 말한다. 통과하지 못한 검증이 있으면 그대로 밝힌다.
